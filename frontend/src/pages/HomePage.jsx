    ...
    <AvatarFallback className="flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 text-sm font-semibold text-slate-800">
      {tutor.name?.split(" ").map((n) => n[0]).join("")}
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 text-sm text-slate-600">
            <div className="flex items-center gap-2"><Star className="h-4 w-4" /> {tutor.rating || 4.5}</div>
          ...
          <div className="mt-6 flex gap-3">
            <Button className="flex-1 rounded-2xl bg-slate-900 px-4 py-3 text-white" onClick={() => onBook(tutor)}>
              Request Demo
            </Button>
            <Button className="flex-1 rounded-2xl border border-slate-300 px-4 py-3" onClick={() => onViewProfile(tutor)}>
              View Profile
            </Button>
          </div>
        </CardContent>
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <section className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:grid-cols-2 md:py-24">
        <div className="flex flex-col justify-center">
          <Badge className="mb-5 w-fit rounded-full bg-white px-4 py-2 text-slate-700 shadow-sm">
            Trusted tutors for school, college, and skills
          </Badge>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <SectionTitle
            ...
            subtitle="Filter by subject, location, category, and mode."
          />
      ...
      <Modal open={!!bookingTutor} onClose={() => setBookingTutor(null)} title="Book Demo Class">
        {bookingTutor ? (
          <div className="space-y-5">
            ...
            <div className="flex gap-3">
              <Button className="rounded-2xl bg-slate-900 px-4 py-3 text-white" onClick={submitBooking}>
                Confirm booking
              </Button>
              <Button className="rounded-2xl border border-slate-300 px-4 py-3" onClick={() => setBookingTutor(null)}>
                Cancel
              </Button>
            </div>
          </div>
