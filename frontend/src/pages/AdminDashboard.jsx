      ...
      {parentEnquiries.length === 0 ? (
        ...
        parentEnquiries.map((p) => (
          <div key={p._id} className="mb-4 rounded-xl bg-white p-4 shadow">
            ...
            <p><b>Preferred Mode:</b> {p.preferredMode}</p>
            <p><b>Preferred Gender:</b> {p.preferredGender}</p>
            <p><b>Preferred Time:</b> {p.preferredTime}</p>
            <p><b>Preferred Days:</b> {p.preferredDays?.join(", ")}</p>

            <div className="mt-3">
              <p className="font-semibold">Wards:</p>
              {p.wards?.map((ward, index) => (
                <div key={index} className="mt-2 rounded-lg bg-slate-50 p-3">
                  <p><b>Ward Name:</b> {ward.wardName || ward.fullName}</p>
                  <p><b>School:</b> {ward.schoolName}</p>
                  <p><b>Class:</b> {ward.classGrade}</p>
                  <p><b>Subjects:</b> {ward.subjectsNeeded}</p>
                  <p><b>Performance:</b> {ward.currentPerformance}</p>
                  <p><b>Special Notes:</b> {ward.specialNeeds}</p>
                </div>
              ))}
            </div>
          </div>
