...
async function run() {
  ...
  // Search for any window action with "match" in name or res_model
  const acts = await rpc(uid, "ir.actions.act_window", "search_read", 
    [["|", ["name", "ilike", "match"], ["res_model", "ilike", "match"]]], 
    { fields: ["id", "name", "res_model", "domain", "context"] }
  );
  console.log("All matching Window Actions:", JSON.stringify(acts, null, 2));
}
